...
const Configure = ({ set, setUltimate, getPin, subscribed }) => {
  ...
  ]); // State to store categories and their descriptions
  ...
  const handleToggleRecording = () => {
    if (questions.length >= 60 && !subscribed) {
      ...
      setTimeout(() => {
        ...
        message.textContent =
          "You have reached the maximum number of questions. Please upgrade to record more.";
        ...
        message.style.transition =
          "opacity 0.05s ease-in, opacity 0.2s ease-out";
    ...
    try {
      ...
      const res = await fetch(
        `https://www.server.speakeval.org/updateconfig?id=${id}&pin=${userId}&length=${
          questions.length
        }&rubric=${categoriesString}&limit=${maxTime}&language=${
      ...
      if (res.ok && !response.error) {
        ...
        toast.error(
          "Failed to register configuration" + response.error
            ? `: ${response.error}`
            : ""
        );
      }
  ...
  const handleConfigClick = (config) => {
    ...
    config.questions.map(async (question) => {
      ...
      const blob = await fetch(`data:audio/wav;base64,${question.audio}`).then(
        (res) => res.blob()
      );
  ...
  return (
    <DndProvider backend={HTML5Backend}>
      {!isConfigRegistered && selected ? ( // Added warning banner
          ...
          <div className="flex items-start">
            ...
            <div className="ml-3">
              <p className="text-sm font-medium">
                <strong>Warning:</strong> Your configuration has not been update
                yet. Click "Update" at the bottom of the page to save your work.
              </p>
            </div>
          </div>
        </div>
      ...
      <div>
        <style>{styles}</style>
        {showUpgrade && (
          <Upgrade onClose={() => setShowUpgrade(false)} doc={document} />
        )}
        {showAutofillUpgrade && (
