...
function CreateRoom({ initialUserId = '' }) {
    ...
    useEffect(() => {
        const fetchConfigs = async () => {
            ...
            try {
                const res = await fetch(`https://backend-g5fp.onrender.com/getconfigs?pin=${userId}`);
    ...
    const handleConfigSubmit = async () => {
        ...
        try {
            const res = await fetch(`https://backend-g5fp.onrender.com/create_room?code=${time}&pin=${userId}&config=${configId}`);
            const parsedData = await res.json();
