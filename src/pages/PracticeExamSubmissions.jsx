...
const ResponseViewer = ({ examCode, studentName, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <Card color="purple" className="w-full max-w-4xl max-h-[70vh]">
        ...
        {/* Footer */}
        <div className="mt-6 mb-10 pt-4 border-t border-purple-500/30">
...
const PracticeExamSubmissions = () => {
  ...
  const fetchSubmissions = async () => {
    try {
      ...
      if (!data.submissions) {
        setError("Invalid response format - no submissions data");
        setLoading(false);
        return;
      }

      // Filter submissions for this specific exam
      const examSubmissions = data.submissions.filter(s => s.examCode === examCode);
      
      setSubmissions(examSubmissions);
      
      // Get exam info from first submission
      if (examSubmissions.length > 0) {
        setExamInfo({
          code: examSubmissions[0].examCode,
          configName: examSubmissions[0].configName
        });
      }
  ...
  return (
    <div className="min-h-screen text-gray-200 font-sans">
              ...
              <div>
                ...
                {examInfo && (
                  <p className="text-lg text-gray-300 mt-2">
                    {examInfo.configName} ({examInfo.code})
                  </p>
                )}
              </div>
          ...
          <Card>
            {submissions.length === 0 ? (
              <div className="text-center py-12">
                <p className="text-xl text-gray-400">No submissions found for this practice exam.</p>
                <p className="text-gray-500 mt-2">
                  Students need to complete this practice exam for submissions to appear here.
                </p>
              </div>
            ) : (
                  ...
                  >
                    ...
                    <div className="flex items-center space-x-6 w-full sm:w-auto">
                      ...
                      <div className="flex space-x-2">
                        <button
                          onClick={() => handleViewResponses(submission.studentName)}
                        ...
                        <button
                          onClick={() => navigate(`/practice-exam-responses/${examCode}/${encodeURIComponent(submission.studentName)}`)}
      ...
      {showResponseViewer && selectedStudent && (
        <ResponseViewer
          examCode={examCode}
...
export default PracticeExamSubmissions;
