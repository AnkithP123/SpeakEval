...
function TeacherPortalRoom({ initialRoomCode, pin }) {
  ...
  // New state for the single email modal
  ...
  const fetchTotalQuestions = async () => {
    try {
      const response = await fetch(
        `https://www.server.speakeval.org/get_num_questions?code=${roomCode}`
      );
  ...
  const fetchParticipants = async () => {
    ...
    try {
      const response = await fetch(
        `https://www.server.speakeval.org/downloadall?code=${roomCode}`,
        {
          headers: {
  ...
  const fetchNextPrevious = async (code) => {
    try {
      const response = await fetch(
        `https://www.server.speakeval.org/get_next_previous?code=${code}`
      );
      const data = await response.json();
      if (data.error) {
        toast.error(data.error);
  ...
  // New function to handle showing the single email modal
  const handleShowSingleEmailModal = (emailData) => {
    setSingleEmailData(emailData);
    setShowSingleEmailModal(true);
  };

  // New function to handle sending a single email
  const handleSendSingleEmail = async () => {
    try {
      ...
      if (resp.success) {
        ...
        // Update failed emails set to remove this student if it was previously failed
      ...
      allData.forEach(({ code, participants }) => {
        participants.forEach((participant) => {
          ...
          participantMap
            ...
            .questionData.set(code, participant);
        });
      });
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-900 via-blue-900 to-slate-900">
      ...
      {/* Single Email Modal - Now rendered at the root level */}
