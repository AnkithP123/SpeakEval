...
function JoinRoomContent() {
  ...
  const handleNextStep = async () => {
    ...
    const res = await fetch(`https://www.server.speakeval.org/verify_room_code?code=${code}`)
    const parsedData = await res.json()
    setLoading(false)
    if (parsedData.valid) {
      setStep(2)
      ...
      <Card color="purple" className="w-full max-w-md mx-auto">
        ...
        {step === 1 ? (
          <div className="w-full mb-4 space-y-6">
            <div>
              <label className="block text-lg font-semibold mb-2 text-white" htmlFor="roomCode">
                Room Code
              </label>
              <input
                id="roomCode"
          ...
          <div className="w-full space-y-6">
            ...
            <button
              ...
              disabled={loading}
            >
              <span className="relative z-10">{loading ? "Loading..." : "Join"}</span>
            </button>
          </div>
        )}
      </Card>
