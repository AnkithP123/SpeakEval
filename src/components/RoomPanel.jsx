...
function RoomPanel({ roomCode, userId, config, setRoomCodes }) {
    ...
    const fetchParticipants = async () => {
      try {
        ...
        if (newCheaters.length > 0) {
          toast.error(
            "Some participants broke test integrity: " + newCheaters.join(", ")
          );
        }
        if (
          data.cheaters &&
          data.cheaters.length > 0 &&
          newCheaters.length > 0
        ) {
  ...
  const sendStatus = async () => {
    ...
    try {
      const response = await fetch(
        `https://www.server.speakeval.org/check_status?code=${recordingRoomCode}&participant=${
          selectedStudent || newStudentName
        }&uuid=${studentUuid}`
      );
      ...
      switch (responseCode) {
        case 5:
          ...
          if (isRecording) {
            stopRecording();
            setError(
              "Time limit reached. Your recording has been automatically stopped and uploaded."
            );
            setIsError(false);
          }
          break;
  ...
  const handleNewQuestion = async () => {
    ...
    try {
      const response = await fetch(
        `https://www.server.speakeval.org/new_question?code=${recordingRoomCode}&pin=${userId}`
      );
      const data = await response.json();
  ...
  const joinRoomAsNewStudent = async (studentName) => {
    ...
    try {
      ...
      const res = await fetch(
        `https://www.server.speakeval.org/join_room?code=${roomCode}&participant=${studentName}`
      );
  ...
  const handleNextStep = async () => {
    ...
    if (!selectedStudent && !newStudentName) {
      toast.error(
        "Please select an existing student or enter a new student name"
      );
      return;
    }
    ...
    if (selectedStudent && newStudentName) {
      toast.warning(
        "Both student selection and new name are provided. Using selected student."
      );
      setNewStudentName("");
    }
  ...
  const getAudio = async () => {
    if (!audioBlobURL && recordingRoomCode) {
      ...
      try {
        const response = await fetch(
          `https://www.server.speakeval.org/receiveaudio?code=${recordingRoomCode}&participant=${
            selectedStudent || newStudentName
          }&number=1`
        );
        if (!response.ok) {
  ...
  const startRecording = async () => {
    ...
    try {
      ...
      try {
        await fetch(
          `https://www.server.speakeval.org/started_playing_audio?code=${recordingRoomCode}&participant=${
            selectedStudent || newStudentName
          }&time=${currentTime}`
        );
      } catch (error) {
  ...
  const upload = async (formData) => {
    ...
    try {
      ...
      const uploadUrlResponse = await fetch(
        `https://www.server.speakeval.org/get-recording-upload-url?code=${recordingRoomCode}&participant=${
          selectedStudent || newStudentName
        }`,
        {
          method: "GET",
      ...
      const response = await fetch(
        `https://www.server.speakeval.org/upload?code=${recordingRoomCode}&participant=${
          selectedStudent || newStudentName
        }&index=${questionIndex}&token=${studentUuid}`,
        {
          method: "POST",
          headers: {
    ...
    <div className="container mx-auto px-4 py-8">
      <Card color="cyan" className="relative min-h-[80vh]">
        <div className="flex justify-between items-start mb-6">
          <div>
            <div className="bg-black/30 text-white rounded-lg p-3 flex items-center w-fit">
          ...
          <div className="flex flex-col items-end">
            ...
            {config?.configType !== "Simulated_Conversation" && (
              <button
            ...
            {roomStarted && (
              <div className="flex flex-col items-end mt-3">
                <div className="bg-black/30 text-white rounded-lg p-3 flex items-center w-fit mb-3">
                  <FaHashtag className="text-cyan-400 mr-2" /> Questions in Set:{" "}
                  {config.questions.length}
                </div>
                <div className="bg-black/30 text-white rounded-lg p-3 flex items-center w-fit">
                  <FaQuestion className="text-cyan-400 mr-2" /> Questions Given:
                  {` ${questionsGiven}`}
                </div>
              </div>
            )}
              ...
              <div className="space-y-6">
                <div className="bg-black/20 p-4 rounded-lg border border-green-500/30">
                  <p className="text-white text-center">
                    First, select an existing student or enter a new student
                  ...
                  {error && (
                    <div
                      className={`mt-6 p-4 rounded-lg border ${
                        isError
                          ? "bg-red-100/30 border-red-500/30 text-red-100"
                          : "bg-green-100/30 border-green-500/30 text-green-100"
                      }`}
                    >
                      <p>{error}</p>
                    </div>
